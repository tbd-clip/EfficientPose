...
def focal(alpha=0.25, gamma=1.5):
    ...
    def _focal(y_true, y_pred):
        ...
        #labels = labels[indices, :].assign(tf.constant([1, 0, 0, 0, 0, 0, 0, 0]))
        #labels[indices].assign(tf.constant([1, 0, 0, 0, 0, 0, 0, 0]))
        #single_class_labels = tf.zeros_like(labels)
...
def transformation_loss(model_3d_points_np, bcube_priors_np, num_rotation_parameter):
    ...
    def _transformation_loss(y_true, y_pred):
        ...
        gt_scaling = tf.gather_nd(gt_scaling, indices)
        ...
        transformed_points_target = rotate(bcube_prior_points * gt_scaling, axis_target, angle_target) + regression_target_translation
        ...
        sym_indices = tf.Print(sym_indices, [tf.shape(sym_indices)], message=" sym_indices.shape: ", summarize=-1)
        ...
        sym_points_pred = tf.Print(sym_points_pred, [tf.shape(sym_points_pred)], message=" sym_points_pred.shape: ", summarize=-1)
